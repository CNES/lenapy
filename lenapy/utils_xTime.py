...
def function_climato(t,a,b,c,d,e,f):
        l=2.*np.pi/(LNPY_DAY_YEAR*LNPY_SECONDS_DAY*1.e9)
...
def climato(data, signal=True, mean=True, trend=True, cycle=False, return_coeffs=False,time_period=slice(None,None)):
    ...
    if return_coeffs:
        return res,xr.Dataset({'Year_amplitude':np.sqrt(a**2+b**2),
                       'Day0_YearCycle':np.mod(np.arctan2(b,a)/2./np.pi*LNPY_DAY_YEAR,LNPY_DAY_YEAR),
                       'HalfYear_Amplitude':np.sqrt(c**2+d**2),
                       'Day0_HalfYearCycle':np.mod(np.arctan2(d,c)/2./np.pi*LNPY_DAY_YEAR,LNPY_DAY_YEAR/2.),
                       'Origin':e+d_mean,
                       'Trend':f*LNPY_DAY_YEAR*LNPY_SECONDS_DAY*1.e9
                      })
    else:
        return res
...
def generate_climato(time, coefficients, mean=True, trend=False, cycle=True):
    ...
    if trend:
        f=coefficients.Trend/(LNPY_DAY_YEAR*LNPY_SECONDS_DAY*1.e9)
    if cycle:
        a=coefficients.Year_amplitude*np.cos(coefficients.Day0_YearCycle/LNPY_DAY_YEAR*2*np.pi)
        b=coefficients.Year_amplitude*np.sin(coefficients.Day0_YearCycle/LNPY_DAY_YEAR*2*np.pi)
        c=coefficients.HalfYear_Amplitude*np.cos(coefficients.Day0_HalfYearCycle/LNPY_DAY_YEAR*2*np.pi)
        d=coefficients.HalfYear_Amplitude*np.sin(coefficients.Day0_HalfYearCycle/LNPY_DAY_YEAR*2*np.pi)
...
def to_datetime(data,input_type,format=None):
    ...
    if input_type=='frac_year':
        data['time']=[ 
            pd.to_datetime(f'{int(np.floor(i))}')+pd.to_timedelta(float((i-np.floor(i))*LNPY_DAY_YEAR),unit='D') 
            for i in data.time]
...
def diff_3pts(data,dim,datetime_unit=None):
    ...
    if datetime_unit==None:
        return res
    else:
        return res*(pd.to_timedelta("1%s"%datetime_unit).asm8.astype('float'))
