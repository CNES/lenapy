...
class OceanSet():
    ...
    def ohc(self):
        if NoneType(self.ohc_):
            self.ohc_ = proprietes(self.heat.lnocean.integ_depth(),
    ...
    @property
    def gohc(self):
        return proprietes(self.ohc.lngeo.mean(['latitude','longitude'],weights=['latitude']),
                         'gohc','Global ocean heat content wrt to ocean surface area','J/m²')

    @property
    def msl(self):
        return proprietes(self.ssl.lngeo.mean(['latitude','longitude'],weights=['latitude']),
                         'msl','Mean ocean sea level anomaly','m')
    
    @property
    def tmsl(self):
        return proprietes(self.tssl.lngeo.mean(['latitude','longitude'],weights=['latitude']),
                         'tmsl','Mean thermosteric ocean sea level anomaly','m')
    
    @property
    def hmsl(self):
        return proprietes(self.hssl.lngeo.mean(['latitude','longitude'],weights=['latitude']),
                         'hmsl','Mean halosteric ocean sea level anomaly','m')

    
    @property
    def gohc_TOA(self):
        return proprietes(self.ohc.lngeo.mean(['latitude','longitude'],weights=['latitude'],na_eq_zero=True),
    ...
    def gohc_above(self,target,na_eq_zero=False,mask=True):
        return proprietes(self.ohc_above(target).lngeo.mean(['latitude','longitude'],weights=['latitude'],na_eq_zero=na_eq_zero,mask=mask),
    ...
    def mld_theta0(self):
        ...
        mld1=self.PT.lngeo.isosurface(theta0-self.oml_theta0_threshold,"depth",upper=True)
        mld2=self.PT.lngeo.isosurface(theta0+self.oml_theta0_threshold,"depth",upper=True)
    ...
    def mld_theta0minus_only(self):
        ...
        theta0=self.PT.interp(depth=10).drop("depth")
        self.mld_theta0_ = self.PT.lngeo.isosurface(theta0-self.oml_theta0_threshold,"depth",upper=True).\
    ...
    def mld_sigma0(self):
        ...
        self.mld_sigma0_=self.sigma0.lngeo.isosurface(sigma0+self.oml_sigma0_threshold,"depth",upper=True).\
    ...
    def mld_sigma0var(self):
        ...
        self.mld_sigma0var_=self.sigma0.lngeo.isosurface(ref.lnocean.sigma0.drop("depth"),"depth",upper=True).\
...
class OceanArray():
    ...
    def above(self,depth,**kwargs):
        ...
        return self.cum_integ_depth().lnocean.add_value_surface(0.).interp({'depth':d},**kwargs)
