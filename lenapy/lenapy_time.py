...
class TimeSet:
    ...
    def filter(self, filter_name='lanczos',q=3, **kwargs):
        ...
        **kwargs :
        ...
        for var in self._obj.data_vars:
            if 'time' in self._obj[var].coords:
                res[var]=self._obj[var].lntime.filter(filter_name=filter_name,q=q,**kwargs)
            else:
                res[var]=self._obj[var]
        return xr.Dataset(res)
...
class TimeArray:
    ...
    def OLS(self,degree,tref=None,sigma=None,datetime_unit="s"):
        ...
        """
        est=estimator(self._obj,degree,tref=tref,sigma=sigma,datetime_unit=datetime_unit)
        est.OLS()
        return est
